...
const createHref = computed(() => `/secrets/create?group=${props.group.id}`);

const subtitle = computed(() => {
    const total = props.secrets.total ?? 0;

    if (total === 0) {
        return 'В этой группе пока нет секретов';
    }

    if (props.secrets.last_page > 1) {
        return `${pluralize(total, ['секрет', 'секрета', 'секретов'])} · страница ${props.secrets.current_page} из ${props.secrets.last_page}`;
    }

    return pluralize(total, ['секрет', 'секрета', 'секретов']);
});
...
function updateTags(tags) {
    router.patch(`/secret-groups/${props.group.id}/tags`, { tags }, {
        preserveScroll: true,
        preserveState: true,
    });
}
...
function destroyGroup() {
    if (!confirm(`Удалить группу «${props.group.name}»? Секреты останутся без группы.`)) {
        return;
    }

    router.delete(`/secret-groups/${props.group.id}`);
}
</script>
...
<template>
    <AppLayout :title="group.name" :subtitle="subtitle" max-width="max-w-5xl">
        ...
        <SecretsTable
            ...
            empty-text="Добавьте секрет в эту группу"
